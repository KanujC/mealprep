  ...
  return (
    <ToastContext.Provider value={{ toast }}>
      ...
      <div className="fixed bottom-4 right-4 flex flex-col gap-2 z-[100]">
        {toasts.map((t) => (
          <div key={t.id} className={cn("flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium animate-slide-in max-w-xs",
            t.type === "success" && "bg-green-50 text-green-800 border border-green-200 dark:bg-green-900/40 dark:text-green-200 dark:border-green-800",
            t.type === "error" && "bg-red-50 text-red-800 border border-red-200 dark:bg-red-900/40 dark:text-red-200 dark:border-red-800",
            t.type === "info" && "bg-[var(--accent)] text-[var(--accent-foreground)] border border-[var(--border)]"
          )}>
