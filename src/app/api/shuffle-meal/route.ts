...
export async function POST(req: NextRequest) {
  ...
  await supabase.from("meals").update({ dish_id: newDish.id, is_leftover_of: null }).eq("id", mealId);
  const kanujMM = await supabase.from("meal_members").select("id").eq("meal_id", mealId).eq("member_id", kanuj.id).single();
  const anshiaMM = await supabase.from("meal_members").select("id").eq("meal_id", mealId).eq("member_id", anshia.id).single();
  if (kanujMM.data) await supabase.from("meal_members").update({ calories: newDish.base_calories_kanuj || null, includes_chicken: newDish.allows_chicken_addon, is_paneer_swap: false }).eq("id", kanujMM.data.id);
  if (anshiaMM.data) await supabase.from("meal_members").update({ calories: newDish.base_calories_anshia || null, includes_chicken: false, is_paneer_swap: newDish.paneer_swap }).eq("id", anshiaMM.data.id);
